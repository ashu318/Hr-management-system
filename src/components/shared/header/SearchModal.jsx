      ...
      <div className="dropdown-menu dropdown-menu-end nxl-h-dropdown nxl-search-dropdown">
        ...
        <div className="search-items-wrapper">
          ...
          <div className="recent-result px-4 py-2">
            ...
            {recnetSearch.map(({ search_title, id, path, icon, badge }) => (
              <Card
                key={id}
                icon={icon}
                subTitle={path}
          ...
          <div className="users-result px-4 py-2">
            ...
            {userSearch.map(({ name, id, email, src }) => (
              <Card
                key={id}
                src={src}
                subTitle={email}
          ...
          <div className="file-result px-4 py-2">
            ...
            {filesList.map(({ file_name, id, size, src }) => (
              <Card
                key={id}
                src={src}
                subTitle={size}
                title={file_name}
                badge={<FiDownload size={12} />}
              />
            ))}
          </div>
          <div className="dropdown-divider mt-3 mb-0"></div>
          <Link href={"#"} className="p-3 fs-10 fw-bold text-uppercase text-center d-block">
            Loar More
          </Link>
        </div>
      </div>
...
const Title = ({ name, number }) => {
  return (
    <h4 className="fs-13 fw-normal text-gray-600 mb-3">
      {name} <span className="badge small bg-gray-200 rounded ms-1 text-dark">{number}</span>
    </h4>
  );
};
  ...
  return (
    <div className="d-flex align-items-center justify-content-between hr-card">
      <div className="d-flex align-items-center gap-3">
        {icon ? (
          <div className="avatar-text rounded">{icon}</div>
        ) : (
        ...
        <div>
          <Link href={"#"} className="font-body fw-bold d-block mb-1">
            {title}
          </Link>
      ...
      {icon ? (
        <Link href={"#"} className="badge border rounded text-dark">
          {badge}
          <i className="ms-1 fs-10">
            <FiCommand />
          </i>
        </Link>
      ) : (
        <Link href={"#"} className="avatar-text avatar-md">
          {" "}
          {badge}{" "}
        </Link>
      )}
