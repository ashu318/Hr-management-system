  ...
  return (
    <div className="col-xxl-4 col-xl-6 col-lg-4 col-sm-6 single-note-item all-category">
      <div className="card card-body mb-4 stretch stretch-full">
        <span
          className={`side-stick bg-${getColor(selectCategory.id == id ? selectCategory.name : category)}`}
        ></span>
        <h5
          className="note-title text-truncate w-75 mb-1 d-flex align-items-center"
          data-noteheading={title}
        >
          {title}
          <i
            className={`point ms-2 fs-7 text-${getColor(selectCategory.id == id ? selectCategory.name : category)}`}
          >
            <BsCircleFill />
          </i>
        </h5>
        ...
        <div className="d-flex align-items-center gap-1">
          <span
            className={`avatar-text avatar-sm ${favourites.includes(id) ? "favourite" : ""}`}
