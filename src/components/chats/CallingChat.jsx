...
const CallingChat = () => {
  ...
  return (
    <div className="d-none d-lg-block chat-calling-text-message-sidebar active">
      ...
      <div
        className="d-flex flex-column position-relative in-call-messages-items"
        data-scrollbar-target="#psScrollbarInit"
      >
        <PerfectScrollbar>
          {chatMessageData.map(
            ({ isReplay, messages, time, user_img, user_name, isTyping }, index) => {
              return (
                <div key={index} className="p-4 text-dark border-bottom single-item">
                  <div className="w-100 d-flex align-items-start justify-content-between">
                    <div className="d-flex align-items-start">
                      ...
                      <div>
                        <a href="#" className="fs-13 fw-bold d-flex align-items-center mb-1">
                          {user_name}
                        </a>
                        {messages.map((message, index) =>
                          message.fileUrl ? (
                            ...
                            <div
                              key={index}
                              className="fs-12 fw-normal text-muted text-truncate-2-line"
                    ...
                    {isTyping ? (
                      ...
                      <Dropdown dropdownItems={chatptions} dropdownParentStyle={"ms-3"} />
                    )}
                  </div>
                </div>
              );
      ...
      <div className="px-3 py-2 fs-16 fw-bold">
        <div className="position-relative  ">
          <input
            className="form-control border-0 emoji-picker"
            placeholder="Type your message here..."
          />
          <div
            className="position-absolute emoji-clicker emoji fs-18 c-pointer"
          ...
          {showEmojiPicker && (
            <div className="emoji" ref={emojiPickerRefs}>
              {" "}
              <EmojiPicker
                onEmojiClick={(e) => setEmoji([...emoji, e.emoji])}
