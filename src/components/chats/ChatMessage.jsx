...
const ChatMessage = ({ avatar, name, time, messages, isReplay, isTyping }) => {
  return (
    <div className={`single-chat-item mb-5`}>
      ...
      <div
        className={`wd-500 p-3 rounded-5 bg-gray-200 message-content ${isReplay ? "ms-auto" : ""}`}
      >
        {messages.map((message, index) =>
          message.fileUrl ? (
            ...
            <p
              key={index}
              className="py-2 px-3 rounded-5 bg-white"
...
export const FileMessage = ({ fileUrl }) => {
  return fileUrl.map(({ iconSrc, fileName, fileSize }, index) => (
    <div
      key={index}
    ...
    >
      <div className="d-flex align-items-center">
        ...
        <div className="d-block ms-3">
          <a href="#" className="fs-13 fw-700 text-dark d-block">
            {fileName}
          </a>
